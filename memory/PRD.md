...
- **Saku Aman + payday (selesai, 2026-09-20)**: `PUT /auth/payday` (1-31, fallback kalender kalau kosong) + `GET /saku-aman` (safe_amount = total budget - transaksi expense bulan keuangan berjalan - obligations belum lunas jatuh tempo sebelum gajian; `configured:false` kalau belum ada budget sama sekali, biar user baru tidak lihat angka minus). 9 test baru, 90/90 backend test lulus. Frontend: kartu "Saku Aman" di puncak dashboard (Beranda) + section "Siklus Gajian" di Account buat atur tanggal gajian. Diverifikasi manual: budget 1.5jt - transaksi 75rb → kartu tampil Rp1.425.000, sesuai rumus.
- **Tabungan & Goals (selesai, 2026-09-20)**: `savings_goals`+`savings_deposits` baru di backend. Endpoint `GET/POST /goals`, `GET/PUT/DELETE /goals/{id}`, `POST /goals/{id}/deposit`, `GET /goals/{id}/deposits`. Dua `kind`: `goal` (bebas) dan `annual_fund` (dana tagihan tahunan, **opt-in manual** — wajib `linked_obligation_id` menunjuk ke obligation existing, tidak pernah auto-dibuat, sesuai keputusan §7 #3 di `docs/DATA_MODEL.md`). `suggested_monthly_deposit` dihitung on-the-fly dari `(target-current)/bulan_tersisa`, tidak disimpan. Freemium: max 3 goal aktif di Free (`PLANS["free"]["max_goals_active"]`, sudah ada dari Langkah 4). 11 test baru, 101/101 backend test lulus. Frontend: layar baru `/goals` (list + kartu progress + tombol "Setor" per goal + modal setor cepat) dan `app/goals/form.tsx` (create/edit, pilih `kind`, kalau `annual_fund` pilih dari daftar obligation `billing_cycle=yearly` yang auto-isi nama/target/tenggat). `/goals` sengaja diletakkan di dalam grup route `(tabs)` sebagai tab tersembunyi (`href: null`) — bukan tab bottom-bar ke-6 (5 tab sudah penuh), tapi tetap dapat sidebar desktop; dijangkau dari baris "Tabungan & Goals" di Account (mobile) dan item sidebar baru (desktop). Diverifikasi manual di browser: buat goal bebas → setor → progress bar update; buat obligation tahunan → buat annual_fund goal tertaut → auto-fill jalan; limit 3 goal free plan → modal upgrade muncul di goal ke-4; edit & hapus (konfirmasi tap-dua-kali) jalan.
- Belum dikerjakan sama sekali: `arisan_*` — masih di tahap desain (`docs/DATA_MODEL.md`), belum ada baris kode. Redesign landing dengan hero "Saku Aman" sekarang bisa lanjut (keputusan user: bangun fitur dulu baru redesign) — semua fitur inti v1 kecuali Arisan sudah selesai.
